...
:position(p), vitesse(v), masse(m){}	//appel au constructeur de copie de Vecteur
...
//Setters
void Particule::setVitesse(Vecteur const& v){vitesse = v;}

//Energie cinétique
double Particule::Ecin() const {return 0.5*masse*vitesse*vitesse;}

//Quantité de mouvement
Vecteur Particule::quantite_mvt() const {return masse*vitesse;}          // Je peux écrire ça comme ça? (mult scalaire...) <- Yep
...
{
	if(epsilon != 0.0)	//vérifier qu'on ne divise pas par 0
	{
		...
		int ay(position.getY()/epsilon);		//mais c'est visuellement plus parlant
	...
	{return false;}				//on considère qu'a la précision des doubles, deux particules n'auront jamais la même position
}
