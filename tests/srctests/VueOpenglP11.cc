...
void Vue_OpenGL::InitSysteme11()
{
	sys.ajouter_particule(new GlHelium(1.0,1.0,1.0,0.0,0.0,0.0));
	sys.ajouter_particule(new GlNeon(1.0,18.5,1.0,0.0,0.2,0.0));
	sys.ajouter_particule(new GlArgon(1.0,1.0,3.1,0.0,0.0,-0.5));
}
...
{
  sys.evolue_vieux(event.GetInterval()/20*dt, 1); 	// le pas de temps n'est pas le vrais :( le problème c'est dès que le pas de temps deviens trop grand ya un segmentation fault qui aparait
